...
// NewSQLPostRepository create new PostRepository type
...
func (r *SQLPostRepository) GetByID(id int) (*Post, error) {
	query := `
		SELECT p.id, p.title, p.url, p.user_id, p.created_at, COALESCE(u.name, '') as user_name,
...
func (r *SQLPostRepository) GetAll(filter Filter) ([]Post, Metadata, error) {
	...
	if filter.Query != "" {
		countQuery += ` WHERE LOWER(p.title) LIKE ?`
		countArgs = append(countArgs, "%"+strings.ToLower(filter.Query)+"%")
	}
	...
	for rows.Next() {
		var post Post
		err := rows.Scan(&post.ID, &post.Title, &post.URL, &post.UserID,
...
func (r *SQLPostRepository) GetComments(postID int) ([]Comment, error) {
	query := `
		SELECT c.id, c.body, c.user_id, c.post_id, COALESCE(u.name, '') as user_name, c.created_at
		FROM comments c
...
func (c *Comment) CreatedAtHuman() string {
	return carbon.NewCarbon(c.CreatedAt).DiffForHumans()
}
...
// GetCommentsCountHuman GetCommentsHuman
func (p *Post) GetCommentsCountHuman() string {
	if p.CommentCount > 1 {
